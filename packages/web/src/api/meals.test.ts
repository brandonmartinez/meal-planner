...
describe("meals api client", () => {
  ...
  it("listMeals combines tags and difficulty facets (AND across facets)", async () => {
    let url = "";
    server.use(
      http.get("/api/families/f-1/meals", ({ request }) => {
        url = request.url;
  ...
  it("listMeals omits tags params when none are provided", async () => {
    let url = "";
    server.use(
      http.get("/api/families/f-1/meals", ({ request }) => {
        url = request.url;
    ...
    await mealsApi.listMeals("f-1", { tags: [] });
    expect(url).not.toContain("tags=");
  });

  it("createMeal includes tags array in the body", async () => {
  ...
  it("updateMeal sends an emptied tags array to clear assignments", async () => {
    let body: { tags?: unknown } = {};
    server.use(
      http.put("/api/families/f-1/meals/m-1", async ({ request }) => {
        body = (await request.json()) as { tags?: unknown };
    ...
    await mealsApi.updateMeal("f-1", "m-1", { name: "Soup", tags: [] });

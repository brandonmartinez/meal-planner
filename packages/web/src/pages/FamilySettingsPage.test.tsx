...
describe('FamilySettingsPage', () => {
  ...
  it('auto-surfaces the meal:image scope checkbox and sends it when selected (#180)', async () => {
    let createdBody: { name?: unknown; scopes?: unknown } | undefined;
    server.use(
      ...parentBase(),
      http.get('/api/families/:id/agent-credentials', () => HttpResponse.json([])),
        ...
        return HttpResponse.json(
          {
            id: 'cred-new',
            name: 'Image Uploader',
            scopes: ['meal_plan:read', 'meal:image'],
            key: 'agent-raw-key-xyz',
            expiresAt: null,
            createdAt: '2026-01-01T00:00:00.000Z',
          },
    ...
    // The new binary-upload scope surfaces as a grantable checkbox purely from
    // the shared scope metadata — no bespoke UI wiring for #180.
    const imageCheckbox = screen.getByRole('checkbox', {
      name: 'Upload meal images',
    });
    ...
    await waitFor(() => expect(createdBody?.name).toBe('Image Uploader'));
    expect(createdBody?.scopes).toEqual(['meal_plan:read', 'meal:image']);
  });

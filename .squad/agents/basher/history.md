...
📌 Team update (2026-07-05T21:05:00Z): Investigated prod 500 on meal-image upload (#181). Root cause: no IMAGE_STORAGE_ROOT, no volume, node user EACCES on root-owned path; silent catch blocks masked the error. Authored PR #182 (single-replica RWO PVC fix in this repo's k8s/) — **WRONG TARGET, PR CLOSED.** **KEY LESSON: production deploys via `raspberry-pi-kubernetes-cluster` (ArgoCD), NOT meal-planner's `k8s/` folder.** The correct fix (Longhorn RWX PVC + IMAGE_STORAGE_ROOT + fsGroup:1000, preserving HPA 2–3 replicas) is filed as `raspberry-pi-kubernetes-cluster` issue #104 and is in-flight with Dallas. Single-replica RWO would break the HPA. — logged by Scribe

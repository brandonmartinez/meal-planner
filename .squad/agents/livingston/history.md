...
## 2026-08-04T10:35:00-04:00 — Slice 2b: authored-layout WRITE path (validator + REST/agent/MCP, no UI)

- **Shared validator** `packages/shared/src/validateAuthoredLayout.ts` — pure `(ingredients, instructions) → {ok:true}|{ok:false;code;message}`, exported with `AUTHORED_LAYOUT_CODES`. Single source of truth for the P2.5 cross-field invariants Zod can't express. Runs on EVERY write (replace-all can desync the pair). Five DISTINCT stable codes (Yen's suite asserts distinctness): `SPAN_PAIR_INCOMPLETE`, `SPAN_OUT_OF_RANGE`, `SPAN_PROCESS_INCOMPLETE`, `SPAN_COLUMN_OVERLAP`, `COLUMN_INVALID`. Cross-column overlap REQUIRED (the cascade) — grouping is per-column, never global; null column normalizes to lane 0. Coverage gaps allowed.
- **API threading** — layout fields on existing objects (no `/matrix` endpoint). `routes/meals.ts` create+update schemas (import stays layout-free via `baseInstructionInputSchema`); `routes/agent.ts` ingredient+instruction schemas (discharges Phase-1 write-side N/A); `services/meals.ts` `mapIngredientCreates`/`mapInstructionCreates` pass-through + `InvalidLayoutError`/`assertValidLayout` before both create and update writes.
- **MCP parity** — `apiClient.ts` Create/UpdateMealInput fields; `tools.ts` schemas with model-facing `.describe()` (indices into ingredient order, inclusive, per-column non-overlap, cross-column overlap = cascade) + updated tool descriptions. Scope unchanged (`meal:write`).
- **Omit-defaulting is load-bearing**: omitted ⇒ null ⇒ derived. `kind ?? undefined` (Prisma default PROCESS), everything else `?? null`. An update that never mentions spans does NOT flip to authored — proven by test. Anti-staleness reaffirmed: `matrixSource` stays structural, no stored boolean, no persisted permutation (#218/#219 class).
- **Reject, never clamp**: invalid ⇒ reject whole write (400 REST / 422 agent). Placeholder ⇒ 403 audited `placeholder`, identical on PUT + PATCH (fixed a pre-existing REST-PUT 500→403 gap). Agent layout rejections audit `invalid_layout:${code}`.
- **DEVIATION (flagged, defensible):** spec P2.4.3 says validate in both routes + defensively in service; I put the AUTHORITATIVE call in the SERVICE only. More parity-correct (one byte-identical call site for REST/agent/MCP) and the partial-update resulting-pair merge needs DB reads routes lack — on an omitted array I load persisted rows and validate the merge before any delete.
- **Effective-pair merge in `updateMeal`**: `data.ingredients ?? findMany({select:{groupLabel}})`, `data.instructions ?? findMany({select:{kind,column,spanFrom,spanTo}})`, then `assertValidLayout` BEFORE deletes. Defensive `?? []` (vitest-mock-extended returns undefined for unstubbed findMany).
- **Mock-factory gotcha**: route tests `vi.mock("../services/meals.js")` replace the module, so `mealService.InvalidLayoutError` was undefined → `instanceof undefined` threw; added a real `InvalidLayoutError` class to each mock factory.
- **Tests**: shared 118 (43 Yen adversarial + 22 mine + 53 pre-existing), mcp 124 (+3), api 1034 (+11). Verified: `pnpm -r run build` clean; `pnpm -r run test` = **1982 pass / 0 fail** (shared 118, mcp 124, web 706, api 1034); `pnpm -r run lint` = **0 errors** (6 pre-existing api `no-explicit-any` warnings + 1 unused-import in Yen's adversarial test, out of my fence). No schema change, no migration. Committed with explicit pathspec.

📌 Team update (2026-08-04T10:45:00-04:00): Rusty **REJECTED** Slice 2b on `importMeals` dangling-span blocker; Livingston locked out as implementation author. Yen fixed it (`a3772aa`, stash-proof verified). Rusty independently reproduced the pre-fix failure before accepting. Rusty then **APPROVED** Slice 2b; PR #223 open. **Livingston AND Yen are now both locked out of this artifact** — further revision → a third api specialist. Service-only validation deviation ratified and recorded in decisions.md. — Scribe cross-cut

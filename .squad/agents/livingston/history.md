...
- Zero-parity-impact CONFIRMED in code: `groupLabel` is display-only and never persisted; no schema/migration change (authored `groupLabel` column stays for Phase 2), no DTO shape change, no #96/MCP change. Only the derived fallback was removed.

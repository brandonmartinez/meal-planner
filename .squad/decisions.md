...
### 2026-07-05: PR #187 imageUrl relative asset-path security review
**By:** Frank (Security)
...
### 2026-07-09: Managed pantry staples implementation (#205, PR #214)
**By:** Saul
**What:** New `PantryStaple` Prisma model (`id`, `name`, `nameNormalized`, `familyId`, `createdAt`, `updatedAt`) with `@@index([familyId])` and `@@unique([familyId, nameNormalized])`, FK to `Family` `onDelete: Cascade`. Family-scoped CRUD service normalizes names on write with the same `normalizeIngredientName` used by `groceryKey`. Routes: `authenticateJWT → requireMembership` for read/list; mutations additionally `requireRole("PARENT")`. Grocery separation is DERIVED at read time — `annotatePantryStaples` fetches the family's staple name-set and sets `isPantryStaple` on each grocery item whose normalized name matches; nothing is persisted on `GroceryItem` and no item is ever deleted/pruned. Web: PARENT-only staples section in Family Settings; grocery list renders a distinct collapsible "Pantry Staples" section (collapsed by default) separate from category groups. Migration authored via `prisma migrate diff --script` (additive-only, 0 DROPs verified); never `migrate dev`.
**Why:** Read-time derivation guarantees "staples are never deleted/pruned" with zero extra guard code and keeps the section correct across regen / removePastDays without touching the #204/#206 orphan logic. Matching via `nameNormalized` (not raw `name`) reuses the exact grocery match key, so case/whitespace-insensitive matching is consistent by construction. Deviations from sibling models, documented for review: (1) `onDelete: Cascade` (task-required) vs siblings' `Restrict` — a staple has no meaning without its family; (2) the task's literal `@@unique([familyId, name])` is satisfied by `@@unique([familyId, nameNormalized])`, the established tested uniqueness pattern, which also enforces case/whitespace-insensitive uniqueness. MCP parity is N/A: pantry staples are a family settings/grocery feature, not part of the recipe/meal agent surface.
